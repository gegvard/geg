...
UExcelCell* UExcelWorksheet::CellAtRef(FExcelCellReference cellReference)const
{
	...
	// Не вызываем reserve(row) — это резервирует N ячеек, а не N строк, и на больших
	// циклах только замедляет. Используйте ReserveCapacity / Batch write.
...
{
	xlnt::cell_reference cr(cellReference.Column, cellReference.Row);
	// reserve(row) был ошибочным: xlnt::reserve резервирует ячейки hashmap, не строки.
	// Для массовой записи используйте ReserveCapacity(Rows*Columns) / WriteStringRow.
	xlnt::cell c = mData.cell(cr);
	c.value(value);
}
